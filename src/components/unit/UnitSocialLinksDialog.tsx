...
export const UnitSocialLinksDialog: React.FC<UnitSocialLinksDialogProps> = ({
  ...
  // 3. Use Field Array for Dynamic List
  ...
  const onSubmit = (data: SocialLinksFormValues) => {
    // FIX: Explicitly map to SocialLink[] to satisfy TypeScript
    const formattedLinks: SocialLink[] = data.links.map((link) => ({
      platform: link.platform,
      url: link.url,
      // If ID is missing (new item), generate one. If present, keep it.
      id: link.id || Date.now().toString() + Math.random().toString().slice(2),
    }));
      ...
      <DialogContent className="max-w-[550px] rounded-2xl max-h-[85vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 mt-2">
          ...
          <Button
            type="button"
            variant="outline"
            onClick={() => append({ platform: "", url: "", id: "" })}
